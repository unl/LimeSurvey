            ...
            foreach ($this->q2subqInfo as $qinfo)
            {
                        ...
                        && isset($qattr['exclude_all_others']) && trim($qattr['exclude_all_others']) != '' && count(explode(';',trim($qattr['exclude_all_others']))) == 1)
                {
                    ...
                    if ($hasSubqs) {
                        ...
                        foreach ($subqs as $sq)
                        {
                            if ($q->availableAttributes('exclude_all_others_auto'))
                            {
                                if ($this->sgqaNaming)
                                {
                ...
                if (isset($qattr['min_answers']) && trim($qattr['min_answers']) != '')
                {
                    ...
                    if ($hasSubqs) {
                        ...
                        foreach ($subqs as $sq) {
                            ...
                            {
                                $sq_name = $q->getAnswerCountSQ($this->sgqaNaming, $sq, true);
                                if (!is_null($sq_name)) $sq_names[] = $sq_name;
                            }
                        }
                        if (count($sq_names) > 0) {
                            if (!isset($validationEqn[$q->id]))
                ...
                if (isset($qattr['max_answers']) && trim($qattr['max_answers']) != '')
                {
                    ...
                    if ($hasSubqs) {
                        ...
                        foreach ($subqs as $sq) {
                            ...
                            {
                                $sq_name = $q->getAnswerCountSQ($this->sgqaNaming, $sq, false);
                                if (!is_null($sq_name)) $sq_names[] = $sq_name;
                            }
                        }
                        if (count($sq_names) > 0) {
                ...
                if (isset($qattr['max_num_value']) && trim($qattr['max_num_value']) != '')
                {
                    ...
                    if ($hasSubqs) {
                        ...
                        $sq_names = array();
                        foreach ($subqs as $sq) {
                            if ($q->availableAttributes('max_num_value'))
                            {
                                if ($this->sgqaNaming)
                                {
                                    $sq_names[] = $sq['rowdivid'] . '.NAOK';
